    ...
    <div class="card-group">
        <div class="card col-4">
            <div class="card-header">
                <input type="text" name="buscar" id="buscar" v-model="buscar" class="form-control"
                       placeholder="Buscar" autocomplete="off">
            </div>
            <div class="card-body">
                <ul class="list-group">
                    <li :class="['list-group-item', 'list-group-item-action', {'active' : dep.nombre === departamento.nombre}]"
                        :key="departamento.id" v-for="departamento in buscarDepartamento"
                        @click="cargarDepartamento(departamento)">
                        {{ departamento.nombre }}
                    </li>
                </ul>
            </div>
        ...
        <div class="card col-8">
            <div class="card-header">
                <button class="btn btn-secondary" @click="nuevoDepartamento">
                    <i class="far fa-file"></i> Nuevo departamento
                </button>
                <button class="btn btn-danger" :disabled="!modoEditar || dep.nombre === 'Sin Departamento'"
            ...
            <div class="card-body">
                <form v-on:submit.prevent="guardarDepartamento" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="nombre" :class="[{'is-invalid' : invalid}, 'form-control']"
                               autocomplete="off" v-model="dep.nombre">
        ...
        computed: {
            buscarDepartamento() {
                return this.departamentos.filter(departamento =>
                    departamento.nombre.toLowerCase().includes(this.buscar.toLowerCase())
                );
            }
        }
    };
